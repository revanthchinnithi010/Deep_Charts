/**
 * CustomChart.tsx — React Native / Skia port (Phase 9.16 Pass B)
 *
 * Pass A scope: candles, price axis, time axis, viewport, camera/transform,
 * visible candle window, basic pan/zoom, chart layout.
 *
 * Pass B scope (this pass): crosshair, tooltip, and the gesture model that
 * drives them. RN has no mouse hover, so unlike the web source (which relies
 * on LWC's native hover crosshair for desktop + a hand-rolled touch state
 * machine layered on top for mobile web) this port only needs the touch
 * half of that state machine:
 *
 *   long-press  → activate crosshair at the touch point (freezes pan/zoom)
 *   drag        → move the crosshair while active (chart stays frozen)
 *   lift finger → crosshair stays locked/visible so the tooltip is readable
 *   tap         → dismiss the locked crosshair (or fitContent on double-tap)
 *
 * The chart shim's public subscribeCrosshairMove / setCrosshairPosition /
 * clearCrosshairPosition (LWC-shaped API, previously Pass A stubs) are now
 * fully wired so future consumers (position tool, drawing overlay, etc.)
 * can read crosshair state the same way the web source does. The module-level
 * pub-sub in @/lib/crosshairState (emitCrosshair/resetCrosshair) continues to
 * carry OHLCV data for zero-React consumers exactly as it does on web.
 *
...
import {
  Canvas, Rect, Line, Group, Fill, Path, Skia, vec, DashPathEffect, Circle,
...
/** Inverse of barIdxToX — nearest bar index under a plot-local x pixel. */
function xToBarIdx(x: number, logicalFrom: number, barW: number): number {
  if (barW <= 0) return Math.round(logicalFrom);
  return Math.round(x / barW + logicalFrom - 0.5);
}
...
/** Fuller "<Mon DD, HH:MM>" / "<Mon DD YYYY>" label for the crosshair time tooltip. */
function formatCrosshairTime(time: number, interval: string): string {
  const d = new Date(time * 1000);
  const isDay = interval === "D" || interval === "1D" || interval === "W" || interval === "1W";
  const date = `${MONTHS[d.getMonth()]} ${String(d.getDate()).padStart(2, "0")}`;
  if (isDay) return `${date} ${d.getFullYear()}`;
  return `${date}, ${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
}

// fmtVolume is imported from @/lib/fmtVolume (moved in Pass C for shared use)
...
class SkiaChartApiImpl implements _ISkiaChart {
  ...
  // ── Crosshair (Pass B) ───────────────────────────────────────────────────────
  // Matches LWC's subscribeCrosshairMove(MouseEventParams) shape:
  //   { point: {x,y}|undefined, time: Time|undefined, seriesData: Map<series,bar> }
  // RN has no mouse hover, so nothing calls this on its own — CustomChart's
  // touch crosshair (long-press + drag) drives it via _fireCrosshairAt().
  private _crosshairListeners: Array<(p: unknown) => void> = [];

  subscribeCrosshairMove(fn: (p: unknown) => void) {
    this._crosshairListeners.push(fn);
  }

  clearCrosshairPosition() {
    this._fireCrosshairAt(null, null, null);
  }

  /** Programmatic positioning by price/time (callers that already know the bar). */
  setCrosshairPosition(_price: number, time: unknown, series: unknown) {
    const bar = this._barsRef.current.find(b => b.time === time) ?? null;
    this._fireCrosshairAt(null, bar, (series as SkiaSeriesApiImpl<string>) ?? null);
  }

  /** Internal — called by CustomChart's touch-crosshair gesture handlers. */
  _fireCrosshairAt(
    point:  { x: number; y: number } | null,
    bar:    OHLCBar | null,
    series: SkiaSeriesApiImpl<string> | null,
  ) {
    const param = bar
      ? {
          ...
          time:  bar.time,
          seriesData: series
            ? new Map([[series, { open: bar.open, high: bar.high, low: bar.low, close: bar.close }]])
            : new Map(),
        }
      : { point: undefined, time: undefined, seriesData: new Map() };
    for (const fn of this._crosshairListeners) fn(param);
  }
...
const CustomChart = memo(function CustomChart({
  ...
  // settingsRef — lets gesture callbacks (updateCrosshairAt) read the latest
  // settings prop without being in their useCallback dep lists.
  ...
  // ── Crosshair state (Pass B/C) ──────────────────────────────────────────────
  // Plain refs, not React state — position updates flow through the same
  // invalidate()/tick mechanism the pan/pinch handlers already use, so a
  // crosshair drag redraws the canvas exactly like a pan does, no extra
  // re-render machinery needed.
  //
  // Pass C: crosshairPxRef now stores a logical anchor (time + price) in
  // addition to the raw pixel position captured at touch time. crosshairInfo's
  // memo derives the rendered pixel x/y from the logical anchor + the current
  // viewport each frame, so the crosshair tracks pan/zoom automatically without
  // any gesture-handler changes.
  const crosshairActiveRef   = useRef(false);
  // true  = gesture started near the crosshair → drag moves the crosshair
  // false = gesture started away  → pan the chart with the crosshair locked
  const crosshairDragModeRef = useRef(false);
  const crosshairPxRef = useRef<{
    x: number; y: number;           // raw pixel at placement (used for initial distance check)
    time:     number | null;        // logical anchor — bar time (null in future space)
    price:    number | null;        // logical anchor — price  (null in future space)
    isFuture: boolean;              // touch is in blank space past the last real bar
  }>({ x: 0, y: 0, time: null, price: null, isFuture: false });
  ...
  // ── Crosshair handlers (Pass B/C) ───────────────────────────────────────────
  //
  // x/y are view-relative pixels from the gesture (same coordinate space as
  // the GestureDetector's child View, i.e. the whole chart including the
  // price/time axis strips) — clamp into the plot area before use.
  //
  // Pass C additions:
  //   • Future/blank-space detection (rawIdx >= bars.length) — shows price line
  //     only, no OHLCV tooltip, vertical line tracks the raw finger position.
  //   • Magnet mode (settings.crosshair === "magnet") — snaps cy to the series
  //     value at the resolved bar using series._coordRef (synced every render).
  //   • Logical anchor stored in crosshairPxRef (time + price) so crosshairInfo
  //     can re-derive pixel position on every render, making the crosshair track
  //     pan/zoom automatically without any extra gesture changes.
  const updateCrosshairAt = useCallback((x: number, y: number) => {
    const chart = chartRef.current;
    if (!chart) return;
    const plotWNow = Math.max(0, chartWRef.current - PRICE_AXIS_W);
    const plotHNow = Math.max(0, chartHRef.current - TIME_AXIS_H);
    const cx = Math.max(0, Math.min(plotWNow, x));
    const cy = Math.max(0, Math.min(plotHNow, y));

    const logFromNow = chart._getFrom();
    const logToNow   = chart._getTo();
    const barsVisNow = logToNow - logFromNow;
    const barWNow    = barsVisNow > 0 && plotWNow > 0 ? plotWNow / barsVisNow : 0;
    const bars       = barsRef.current;

    // Raw (unclamped) bar index — needed to detect future space
    const rawIdx = xToBarIdx(cx, logFromNow, barWNow);

    crosshairActiveRef.current = true;

    // ── Future / blank space ────────────────────────────────────────────────
    if (rawIdx >= bars.length) {
      // No bar under the finger — show price line + vertical line at raw x;
      // no tooltip data. Raw pixels stored (no logical anchor to derive from).
      crosshairPxRef.current = { x: cx, y: cy, time: null, price: null, isFuture: true };
      resetCrosshair();
      chart._fireCrosshairAt(null, null, null);
      invalidate();
      return;
    }

    const idx = Math.max(0, Math.min(bars.length - 1, rawIdx));
    const bar = bars[idx] ?? null;
    if (!bar) {
      crosshairPxRef.current = { x: cx, y: cy, time: null, price: null, isFuture: false };
      crosshairActiveRef.current = false;
      invalidate();
      return;
    }
    ...
    // ── Magnet mode ─────────────────────────────────────────────────────────
    // coordRef is synced to displayMin/displayMax/chartH on every render —
    // no need to pass them in as args.
    const coordRef = mainRef.current?._coordRef;
    const dMin = coordRef?.displayMin ?? 0;
    const dMax = coordRef?.displayMax ?? 100;
    let finalY     = cy;
    let finalPrice = yToPrice(cy, dMin, dMax, plotHNow);

    if (settingsRef.current?.crosshair === "magnet" && coordRef) {
      const isLine = ctRef.current === "line" || ctRef.current === "line_with_markers" || ctRef.current === "area";
      let magnetPrice: number;
      if (isLine) {
        const sData = mainRef.current?._getData();
        const pt    = sData?.find(d => d.time === bar.time);
        magnetPrice = pt ? (pt.value as number) : bar.close;
      } else {
        magnetPrice = bar.close;
      }
      finalY     = priceToY(magnetPrice, dMin, dMax, coordRef.chartH);
      finalPrice = magnetPrice;
    }

    // Store logical anchor (time + price) — crosshairInfo re-derives pixels
    // from this on every render so the crosshair tracks pan/zoom for free.
    crosshairPxRef.current = { x: cx, y: finalY, time: bar.time, price: finalPrice, isFuture: false };
    emitCrosshair({ time: bar.time, open: bar.open, high: bar.high, low: bar.low, close: bar.close, volume: bar.volume });
    chart._fireCrosshairAt({ x: cx, y: finalY }, bar, mainRef.current);
    invalidate();
  }, [invalidate]);
  ...
  // Long-press + drag (touch-only crosshair — see file header comment):
  //   a long-press activates the crosshair and freezes pan/zoom; while
  //   crosshairActiveRef is true, the pan gesture's onUpdate is redirected
  //   to move the crosshair instead of the chart viewport.
  ...
  const handlePanStart = useCallback((translateX: number, translateY: number, startX: number, startY: number) => {
    if (crosshairActiveRef.current) {
      // Decide sub-state: crosshair drag vs pan-with-locked-crosshair.
      // Compute the crosshair's current pixel position from its logical anchor.
      const chart2  = chartRef.current;
      const series2 = mainRef.current;
      const range2  = chart2?.timeScale().getVisibleLogicalRange();
      let chPixX = crosshairPxRef.current.x;
      let chPixY = crosshairPxRef.current.y;
      if (range2 && series2) {
        const plotW2   = Math.max(0, chartWRef.current - PRICE_AXIS_W);
        const plotH2   = Math.max(0, chartHRef.current - TIME_AXIS_H);
        const barsVis2 = (range2.to as number) - (range2.from as number);
        const barW2    = barsVis2 > 0 && plotW2 > 0 ? plotW2 / barsVis2 : 0;
        const cr       = crosshairPxRef.current;
        if (cr.time !== null && barW2 > 0) {
          const idx2 = barsRef.current.findIndex(b => b.time === cr.time);
          if (idx2 >= 0) chPixX = barIdxToX(idx2, range2.from as number, barW2);
        }
        if (cr.price !== null) {
          const dMin = series2._coordRef.displayMin;
          const dMax = series2._coordRef.displayMax;
          chPixY = priceToY(cr.price, dMin, dMax, plotH2);
        }
      }
      const dx = startX - chPixX;
      const dy = startY - chPixY;
      crosshairDragModeRef.current = Math.sqrt(dx * dx + dy * dy) < 24;
      if (crosshairDragModeRef.current) return; // drag mode — crosshair handles it; don't init panState
      // else: pan-with-locked-crosshair — fall through to normal pan init
    } else {
      crosshairDragModeRef.current = false;
    }
    const chart = chartRef.current;
  ...
  const handlePanUpdate = useCallback((translateX: number, translateY: number, x: number, y: number) => {
    if (crosshairActiveRef.current && crosshairDragModeRef.current) { updateCrosshairAt(x, y); return; }
    // crosshair active but NOT in drag mode → pan-with-locked-crosshair: fall through
    const chart  = chartRef.current;
    ...
    invalidate();
  }, [invalidate, updateCrosshairAt]);

  const handlePanEnd = useCallback((velocityX: number, velocityY: number) => {
    if (crosshairActiveRef.current && crosshairDragModeRef.current) return; // crosshair drag — stays locked
    const chart  = chartRef.current;
    const series = mainRef.current;
    const ps     = panStateRef.current;
  ...
  // A tap dismisses an active/locked crosshair (matches the web source's
  // "tap on blank area dismisses locked crosshair" behavior); otherwise a
  // double-tap fits content, same as before.
  const handleTapEnd = useCallback(() => {
    if (crosshairActiveRef.current) { dismissCrosshair(); return; }
    const now = Date.now();
  ...
  }, [invalidate, dismissCrosshair]);

  // Long-press activates the crosshair at the touch point. Any in-flight pan
  // is cancelled first so the chart doesn't jump/snap right as it freezes.
  const handleLongPress = useCallback((x: number, y: number) => {
    ...
    updateCrosshairAt(x, y);
  }, [updateCrosshairAt]);

  // ── RNGH Gestures ─────────────────────────────────────────────────────────
  const panGesture = useMemo(() => Gesture.Pan()
    .minDistance(3)
    .onStart(e => { runOnJS(handlePanStart)(e.translationX, e.translationY, e.x, e.y); })
    .onUpdate(e => { runOnJS(handlePanUpdate)(e.translationX, e.translationY, e.x, e.y); })
  ...
  const tapGesture = useMemo(() => Gesture.Tap()
    .onEnd(() => { runOnJS(handleTapEnd)(); }),
  [handleTapEnd]);

  // maxDistance is large on purpose: once the crosshair activates, the
  // finger keeps moving to drag it — LongPress must not cancel on that
  // movement (RNGH cancels a long-press once the finger exceeds maxDistance
  // from the start point, which defaults to a few pixels).
  ...
  const composed = useMemo(() =>
    Gesture.Simultaneous(Gesture.Race(panGesture, pinchGesture), tapGesture, longPressGesture),
  ...
  // Crosshair (Pass C) — derived render data recomputed every frame.
  //
  // Normal bars: pixel x/y are derived from the stored logical anchor
  // (time + price) using the *current* logFrom/barW/displayMin/displayMax,
  // so the crosshair tracks pan and zoom automatically — no gesture changes
  // needed (pan already calls invalidate(), which triggers this memo).
  //
  // Future/blank space: no logical anchor; raw captured pixels used directly.
  const crosshairInfo = useMemo(() => {
    if (!crosshairActiveRef.current || plotH <= 0 || plotW <= 0) return null;
    const cr = crosshairPxRef.current;

    // ── Future / blank space branch ─────────────────────────────────────────
    if (cr.isFuture) {
      const price = yToPrice(cr.y, displayMin, displayMax, plotH);
      return {
        x: cr.x, y: cr.y,
        bar: null as OHLCBar | null,
        isFuture: true  as const,
        priceLabel: fmtPrice(price, symbol),
        timeLabel:  "",
      };
    }

    // ── Normal bar branch ────────────────────────────────────────────────────
    // Derive pixel position from logical anchor so it tracks pan/zoom.
    if (cr.time === null || cr.price === null || bars.length === 0) return null;
    const barIdx = bars.findIndex(b => b.time === cr.time);
    if (barIdx < 0) return null;
    const bar = bars[barIdx];
    if (!bar) return null;
    const px = barIdxToX(barIdx, logFrom, barW);
    const py = priceToY(cr.price, displayMin, displayMax, plotH);
    return {
      x: px, y: py, bar,
      isFuture: false as const,
      priceLabel: fmtPrice(cr.price, symbol),
      timeLabel:  formatCrosshairTime(bar.time, interval),
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tick, plotW, plotH, bars.length, logFrom, barW, displayMin, displayMax, symbol, interval]);
          ...
          <GestureDetector gesture={composed}>
            <View style={StyleSheet.absoluteFill}>
              ...
              <Canvas style={{ position: "absolute", left: 0, top: 0, width: plotW, height: plotH }}>
                ...
                {/* ── Crosshair (Pass C) — drawn last so it sits above candles/indicators ── */}
                {crosshairInfo && (() => {
                  const cColor = settings?.crosshairColor ?? "rgba(255,255,255,0.5)";
                  const cWidth = settings?.crosshairWidth ?? 1;
                  const cStyle = settings?.crosshairStyle ?? "solid";
                  const dash =
                    cStyle === "dashed" ? [6, 4] :
                    cStyle === "dotted" ? [1, 4] : null;

                  // Marker dot for line / area series at the crosshair bar (Pass C).
                  // Mirrors LWC's crosshairMarkerVisible behaviour — skipped for
                  // candlestick / bar / heikin_ashi and for future-space touches.
                  let markerDot: React.ReactNode = null;
                  if (!crosshairInfo.isFuture && crosshairInfo.bar && series) {
                    const isLineSeries =
                      chartType === "line" ||
                      chartType === "line_with_markers" ||
                      chartType === "area";
                    if (isLineSeries) {
                      const pt = series._getData().find(d => d.time === crosshairInfo.bar!.time);
                      if (pt) {
                        const dotY = priceToY(pt.value as number, displayMin, displayMax, plotH);
                        markerDot = (
                          <Circle cx={crosshairInfo.x} cy={dotY} r={4} color={upCol} />
                        );
                      }
                    }
                  }
                  ...
                  return (
                    <Group>
                      {/* Vertical line — tracks raw finger x in future space */}
                      <Line p1={vec(crosshairInfo.x, 0)} p2={vec(crosshairInfo.x, plotH)} color={cColor} strokeWidth={cWidth}>
                        {dash && <DashPathEffect intervals={dash} />}
                      </Line>
                      {/* Horizontal price line — always shown */}
                      <Line p1={vec(0, crosshairInfo.y)} p2={vec(plotW, crosshairInfo.y)} color={cColor} strokeWidth={cWidth}>
                        {dash && <DashPathEffect intervals={dash} />}
                      </Line>
                      {markerDot}
                    </Group>
              ...
              <View style={[styles.priceAxis, { width: PRICE_AXIS_W, height: plotH }]}>
                ...
                />
                {/* Crosshair price label (Pass B) */}
                {crosshairInfo && (
                  <View
                    style={[styles.crosshairPriceLabel, { top: Math.max(0, Math.min(plotH - 16, crosshairInfo.y - 8)) }]}
                    pointerEvents="none"
                  >
                    <RNText style={styles.crosshairPriceLabelText}>{crosshairInfo.priceLabel}</RNText>
                  </View>
                )}
              </View>
              ...
              <View style={[styles.timeAxis, { height: TIME_AXIS_H, width: plotW }]}>
                ...
                {/* Crosshair time label (Pass C) — hidden in future space (no bar time) */}
                {crosshairInfo && !crosshairInfo.isFuture && crosshairInfo.timeLabel && (
                  <View
                    style={[styles.crosshairTimeLabel, { left: Math.max(0, Math.min(plotW - 74, crosshairInfo.x - 37)) }]}
                    pointerEvents="none"
                  >
                    <RNText style={styles.crosshairTimeLabelText}>{crosshairInfo.timeLabel}</RNText>
                  </View>
                )}
              </View>
              ...
              {/* ── Crosshair OHLCV tooltip (Pass C) — hidden in future space ── */}
              {crosshairInfo && !crosshairInfo.isFuture && crosshairInfo.bar && (() => {
                const b = crosshairInfo.bar!;
                const bull = b.close >= b.open;
                const ohlcCol = bull ? upCol : downCol;
                return (
                  <View style={styles.crosshairTooltip} pointerEvents="none">
                    <RNText style={styles.crosshairTooltipText}>
                      <RNText style={styles.crosshairTooltipLabel}>O</RNText>
                      <RNText style={{ color: ohlcCol }}> {fmtPrice(b.open, symbol)}  </RNText>
                      <RNText style={styles.crosshairTooltipLabel}>H</RNText>
                      <RNText style={{ color: ohlcCol }}> {fmtPrice(b.high, symbol)}  </RNText>
                      <RNText style={styles.crosshairTooltipLabel}>L</RNText>
                      <RNText style={{ color: ohlcCol }}> {fmtPrice(b.low, symbol)}  </RNText>
                      <RNText style={styles.crosshairTooltipLabel}>C</RNText>
                      <RNText style={{ color: ohlcCol }}> {fmtPrice(b.close, symbol)}  </RNText>
                      <RNText style={styles.crosshairTooltipLabel}>Vol</RNText>
                      <RNText style={{ color: TEXT_COLOR }}> {fmtVolume(b.volume)}</RNText>
                    </RNText>
                  </View>
                );
              })()}
...
const styles = StyleSheet.create({
  ...
  crosshairTooltipText: {
    fontSize: 11,
    fontFamily: "monospace",
    letterSpacing: 0.2,
  },
  crosshairTooltipLabel: {
