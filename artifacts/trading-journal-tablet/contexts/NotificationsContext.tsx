/**
 * NotificationsContext — React Native.
 *
 * Migration of: artifacts/trading-journal/src/contexts/NotificationsContext.tsx
 * Phase 11.5 — Notifications Foundation (React → React Native)
 *
 * Web → RN replacements:
 *   None required — this context uses only React primitives (createContext,
 *   useState, useEffect, useRef, useCallback, useMemo) which are identical
 *   in React Native. No browser APIs are used.
 *
 *   useLiveMarketContext is imported from ./LiveMarketContext which now
 *   exports both WsStatus and AlertTriggeredMsg types plus useLiveMarketContext
 *   returning safe defaults for the Phase 6.x stub period.
 *
 * Business logic preserved verbatim:
 *   MAX_NOTIFICATIONS = 60
 *   addNotification — prepends, slices to MAX_NOTIFICATIONS, generates id
 *   alertEvents effect — maps AlertTriggeredMsg → price/zone/trendline notifs
 *   wsStatus effect — maps status transitions → reconnect/error/reconnected notifs
 *   markRead, markAllRead, clearAll — identical state updaters
 *   unreadCount — useMemo over notifications
 *
 * Exported API preserved verbatim:
 *   NotifSeverity       — union type
 *   NotifType           — union type
 *   AppNotification     — interface
 *   NotificationsProvider — named export (provider)
 *   useNotifications    — named export (hook)
 */
